...
1. **Maps `graph_id`** to Graphiti’s isolation mechanism (e.g. group or metadata filter), consistent with one Neo4j database per environment or multi-tenant groups.
2. **Implements** `create_graph`, `set_ontology` / `get_ontology` using Graphiti’s ontology APIs (Pydantic models derived from current JSON ontology or `docs/golden-set/ontology-min.json`).
